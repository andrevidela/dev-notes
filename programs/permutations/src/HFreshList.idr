...
-- Boolean "relation"
public export
0 BRel : Fin n -> Vect n Type -> Type
BRel i tys = index i tys -> index i tys -> Bool

infix 4 #, ##, #?

public export
data HFreshVect : (tys : Vect n Type) -> (neq : (i : Fin n) -> BRel i tys) -> Type

-- The boolean version
public export
BoolRel : (l : Vect n Type) -> (j : Fin n) ->
          {neq : (i : Fin n) -> BRel i l} -> (x : index j l) -> (xs : HFreshVect l neq) -> Bool
-- The type version
public export
TypeRel : (l : Vect n Type) -> (j : Fin n) ->
          {neq : (i : Fin n) -> BRel i l} -> (x : a) -> (xs : HFreshVect l neq) -> Type

public export
data HFreshVect : (tys : Vect n Type) -> (neq : (i : Fin n) -> BRel i tys) -> Type where
  Nil  : HFreshVect [] neq
  (::) : (1 x : a) -> (1 xs : HFreshVect l neq) ->
         {auto 0 fresh : TypeRel l FZ x xs} ->
         HFreshVect (a :: l) (\idx => ?waht)

{-

%name HFreshVect xs, ys, zs

BoolRel {l = []} i x []     = True
BoolRel {l = t :: ts} i x (y :: xs) = (x `neq` y) && BoolRel x xs

TypeRel i x xs = So (BoolRel x  xs)
{-
